...
class TSDFVolume:
  ...
  def __init__(self, vol_bnds, voxel_size, use_gpu=True):
    ...
    self._trunc_margin = 5 * self._voxel_size  # truncation on SDF 5 *
    ...
    self._tsdf_vol_cpu = -1 * np.ones(self._vol_dim).astype(np.float32) #ones
  ...
  def get_volume(self):
    ...
    return self._tsdf_vol_cpu, self._color_vol_cpu, self._vol_bnds
  ...
  def get_mesh(self):
    ...
    """
    tsdf_vol, color_vol, bnds = self.get_volume()
